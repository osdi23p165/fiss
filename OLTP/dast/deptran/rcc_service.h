...
public:
    ...
    // void SlogSendBatch(const uint64_t& start_index, const std::vector<std::pair<txnid_t, std::vector<SimpleCommand>>>& cmds, const std::vector<std::pair<txnid_t, siteid_t>>& coord_sites, const std::vector<std::pair<txnid_t, vector<parid_t>>>& all_touched_shards, rrr::i32* res, rrr::DeferredReply* defer) override;


    void SlogSendBatch(const uint64_t &start_index, const std::vector<std::pair<txnid_t, std::vector<SimpleCommand>>> &cmds, const std::vector<std::pair<txnid_t, siteid_t>> &coord_sites, const std::vector<std::pair<txnid_t, std::vector<parid_t>>> &touched_pars,
                       const std::vector<uint64_t> &barriers,
