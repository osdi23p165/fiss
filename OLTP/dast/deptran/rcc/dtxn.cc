...
bool RccDTxn::WriteColumn(Row *row,
                          ...
                          const Value &value,
                          int hint_flag) {
    verify(sched_->frame_->site_info_ != nullptr);
    siteid_t my_site = sched_->frame_->site_info_->id;

    if (my_site % N_REP_PER_SHARD == 0 && phase_ == PHASE_RCC_COMMIT) {
        ...
        verify(sched_->frame_ != nullptr);
        verify(sched_->frame_->cdc_client_ != nullptr);
       ...
       //     Log_info("p_key_col_name= %s, row_key = %s", p_key_col_name.c_str(), row_key.c_str());
        ...
        sched_->frame_->cdc_client_->TransferUpdate(
                col_id,
                row->get_table()->Name(),
                row_key,
                p_key_col_name,
...
bool RccDTxn::InsertRow(Table *tbl, Row *row) {
    verify(sched_ != nullptr);
    if(frame_ == nullptr){
        frame_ = sched_->frame_;
    }
    verify(frame_ != nullptr);
    verify(sched_->frame_->site_info_ != nullptr);
    siteid_t my_site = sched_->frame_->site_info_->id;
    if (my_site % N_REP_PER_SHARD == 0 && phase_ == PHASE_RCC_COMMIT) {
        //Log_info("Insert Row %lu at time %d", id(), std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count());
        string row_key = "";
        string p_key_col_name = "";
        int64_t row_index = 1;
        auto key_col_id = row->schema_->key_columns_id();
        for (int k_col = 0; k_col < row->schema_->columns_count(); k_col++) {
            string k;
            string col_name = row->schema_->get_column_info(k_col)->name;
            auto value = row->get_column(k_col);
            switch (value.get_kind()) {
            ...
            //Log_info("insert row: %d tbl: %s txn_id: %lu", k_col, tbl_name.c_str(), id());
            sched_->frame_->cdc_client_->TransferUpdate(
                    k_col,
                    tbl_name,
                    row_key,
                    p_key_col_name,
                    row_index,
                    state_transfer::Update_OpType_INSERT,
                    k,
                    id());
        }
    }
...
void RccDTxn::TraceDepUnkKey(vector<RccDTxn *> &unk_key_txns_) {
    ...
    if (parent_dtxn == this) {
        ...
        //hz: we enabled executed txn as our parents.
...
void RccDTxn::TraceDep(Row *row, column_id_t col_id, int hint_flag) {
    ...
    if (parent_dtxn == this) {
        ...
        if (parent_dtxn != nullptr) { //hz: add executed dtxn as our dep
            //if (parent_dtxn->IsExecuted()) {
            //    ;
            //} else {
