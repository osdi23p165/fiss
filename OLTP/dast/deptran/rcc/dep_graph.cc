...
void RccGraph::AddParentsToMap(RccDTxn* new_v, RccDTxn* dtxn, RccGraph* new_graph){
  Log_debug("%s trace %lu 's parents ", __FUNCTION__ , dtxn->id());
  for (auto& pair : dtxn->parents_) {
    ...
    set<uint32_t>::iterator iter = parent_v->partition_.begin();
    bool skip = true;
    verify(parent_v != nullptr);
//    auto weight = pair.second;
    RccDTxn* new_parent_v = new_graph->FindV(parent_v->id());
    if(new_parent_v != nullptr){
      break;
    }
    ...
    Log_debug("%s add pareant edge: %lu is parent of %lu", __FUNCTION__ , new_parent_v->id(), new_v->id());
    if(new_v!=nullptr){
      new_v->AddParentEdge(new_parent_v, 0);
    }
    verify(parent_v->partition_.size() > 0);
    while(iter != parent_v->partition_.end()){
      if(!parent_v->piggybackedShard.test(*iter)){
        skip = false;
      }
      ++iter;
    }
    ...
    iter = parent_v->partition_.begin();
    while(iter != parent_v->partition_.end()){
      parent_v->piggybackedShard.set(*iter);
      ++iter;
    }
    AddParentsToMap(new_parent_v, parent_v, new_graph);
  }
};
void RccGraph::SelectGraphCmtUkn(RccDTxn* dtxn, RccGraph* new_graph) {
  ...
  // TODO Add dep = 2 graph now FIXME
  AddParentsToMap(new_v, dtxn, new_graph);
...
//    }
  ...
  Log_debug("return graph size: %llx", sz); //TODO verify graph size
...
RccDTxn* RccGraph::AggregateVertex(RccDTxn *rhs_dtxn) {
  ...
  lhs_dtxn->fast_path_ = lhs_dtxn->fast_path_ && rhs_dtxn->fast_path_;
  Log_debug("[%s] for txn id %lu, lhs parrents size = %d, status =%hhd; rhs parents size = %d, status = %hhd", __FUNCTION__ , rhs_dtxn->id(), parent_set1.size(), status1,
           parent_set2.size(), status2);
  ...
  if (status2 >= TXN_CMT) {
//    Log_debug("aggregating gt CMT, txnid: %llx, parent size: %d",
...
//        lhs_dtxn->partition_ = rhs_dtxn->partition_;
  ...
  lhs_dtxn->piggybackedShard |= rhs_dtxn->piggybackedShard;
