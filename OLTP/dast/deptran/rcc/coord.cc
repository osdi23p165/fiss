...
void RccCoord::Dispatch() {
  ...
  int cnt = 0;
  //XSTODO: change this to GetAllCMDS();
  auto cmds_by_par = txn->GetReadyCmds();
  Log_info("cmds_by_par size = %d", cmds_by_par.size());
  for (auto& pair: cmds_by_par) {
    ...
    Log_info("par %u, size = %d", par_id, pair.second.size());
...
void RccCoord::Finish(int32_t fast_path_) {
  verify(0);
