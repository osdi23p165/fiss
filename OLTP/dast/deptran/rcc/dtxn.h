...
public:
    ...
    std::bitset<1024> piggybackedShard; //TODO hardcode here, support up to 1024 shards
    i32 touchColsCnt;
    vector<parid_t> all_touched_shards_;
    bool fast_path_ = true;
    // hopefully this makes involve checks faster
    enum InvolveEnum { UNKNOWN,
                       INVOLVED,
                       FOREIGN };
    ...
    std::map<parid_t, std::map<varid_t, Value>> CommitExecAllRdy(parid_t my_parid);

    //return 0 if there is some pieces dep not checked.
    //return 1 if all dep checked.
    bool DispatchExec(parid_t my_parid);

    virtual void DispatchExecute(const SimpleCommand &cmd,
                                 int *res,
                                 map<int32_t, Value> *output);

    virtual void ExecSimpleCmd(const SimpleCommand &cmd,
                               int *res,
                               map<int32_t, Value> *output);
    ...
    void TraceDepUnkKey(vector<RccDTxn *> &);
...
public:
    ...
    std::vector<SimpleCommand> pieces_; //xs: not used before;
    ...
    //xs: directly copied from dast
    //todo: do we still need pending pieces.
    int n_executed_pieces = 0;
    int n_local_pieces = 0;
    virtual std::map<parid_t, std::map<varid_t, Value>> RecursiveExecuteReadyPieces(parid_t my_par_id);

    virtual std::set<parid_t> GetOutputsTargets(std::map<int32_t, Value> *output);
    virtual bool MergeCheckReadyPieces(const std::map<int32_t, Value> &output);
...
inline rrr::Marshal &operator<<(rrr::Marshal &m, const RccDTxn &ti) {
    m << ti.tid_ << ti.status() << ti.partition_ << ti.epoch_ << ti.parents_;
    return m;
}

inline rrr::Marshal &operator>>(rrr::Marshal &m, RccDTxn &ti) {
    int8_t status;
    m >> ti.tid_ >> status >> ti.partition_ >> ti.epoch_ >> ti.parents_;
