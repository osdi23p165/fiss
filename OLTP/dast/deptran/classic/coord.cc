...
void ClassicCoord::do_one(TxnRequest &req) {
    ...
    this->my_txn_is_loacal_ = req.is_local_txn_;
    verify(req.home_region_id != uint16_t(-1));
    this->home_par_id_ = req.home_region_id;
    Log_info("%s called, isQuery = %d, touchColsCnt = %d", __FUNCTION__ , req.isQuery, req.touchColsCnt);
    this->isQuery = req.isQuery;
    this->touchColsCnt = req.touchColsCnt;
    this->txn_type = req.txn_type_;
