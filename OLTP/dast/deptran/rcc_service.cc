...
void ClassicServiceImpl::SlogSendBatch(const uint64_t &start_index, const std::vector<std::pair<txnid_t, std::vector<SimpleCommand>>> &cmds, const std::vector<std::pair<txnid_t, siteid_t>> &coord_sites, const std::vector<std::pair<txnid_t, vector<parid_t>>>& touched_all_shards, 
    ...
    auto *sched = (SchedulerSlog *) dtxn_sched_;
    *res = SUCCESS;
    sched->OnSendBatch(start_index, cmds, coord_sites, touched_all_shards, barriers_local_indices, isQuery, touchColsCnt, txn_type);
    defer->reply();
}
