...
public:
    ...
    //barrier primitives
    std::vector<uint64_t> global_log_barrier_indices_; 
    uint64_t next_send_global_barrier_ = 0;

    void insert_barrier(uint64_t index){
        std::lock_guard<std::recursive_mutex> guard(mu_);
        global_log_barrier_indices_.push_back(index);
    }
