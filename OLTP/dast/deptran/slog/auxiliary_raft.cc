...
void AuxiliaryRaftImpl::SendToAllPars(size_t log_index){
    ...
    vector<parid_t> all_touched_pars; 
    for (auto &pair: cmds_by_par) {
        parid_t par_id = pair.first;
        all_touched_pars.push_back(par_id); 
        regionid_t rid = par_id / Config::GetConfig()->n_shard_per_region_;
        cmds_by_region[rid][par_id] = pair.second;
    }

    vector<uint64_t> barriers; 
    while(next_send_global_barrier_ < global_log_barrier_indices_.size()){ 
        barriers.push_back(global_log_barrier_indices_[next_send_global_barrier_]);
        next_send_global_barrier_++;
    }
        ...
        if (target_site % (Config::GetConfig()->n_shard_per_region_ * N_REP_PER_SHARD) == 0){
            ...
            if (cmds_by_region.count(rid) != 0){
                ...
                Log_debug("sending pieces (map size = %d) to site %hu of region %u, index = %lu",
                         ...
                         rid,
                         log_index);
                Future::safe_release(proxy->async_SlogSendOrderedCRT(cmds_by_region[rid], log_index, coord_site, all_touched_pars, barriers)); 
            }
            else{
                ...
                Log_debug("sending skip message to site %hu of region %u, index = %lu",
                         ...
                         rid,
                         log_index);
                Future::safe_release(proxy->async_SlogSendOrderedCRT(skip, log_index, coord_site, all_touched_pars, barriers));
            }
        }
    }
};
