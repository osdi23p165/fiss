...
public:
    ...
    string GetTouchedTblCols(innid_t inn_id, parid_t par_id, map<parid_t, vector<pair<string, int>>> &cols, int touchColsCnt, bool isQuery);
    ...
    void OnSendBatch(const uint64_t &start_index,
                     ...
                     const std::vector<uint64_t> &barrier_local_indices,
    ...
    void NotifyQueueIndex (size_t i);

    vector<uint64_t> barrier_indices_;
