...
void BrqCommo::SendFinish(parid_t pid,
    ...
    Future::safe_release(proxy->async_BrqCommit(tid, (BrqGraph &) graph, fast_path_, fuattr));
}
...
void BrqCommo::BroadcastCommit(parid_t par_id,
    ...
    for (auto &p : rpc_par_proxies_[par_id]) {
        ...
        if (skip_graph) {
            ...
            Future::safe_release(proxy->async_BrqCommit(cmd_id, graph, fast_path_, fuattr));
        }
    }
}
