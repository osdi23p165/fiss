...
//            Log_info("%s:, %d, %d, %d\n", fcol->name.c_str(), (fcol != nullptr), (fcol->values != nullptr), (fcol->values->size() == 0));
            if ((fcol != nullptr) &&
                (fcol->values != nullptr) &&
                (fcol->values->size() == 0)) {
                // have foreign table
...
// TODO this should be moved to per benchmark class
    ...
    int init_index(std::map <uint32_t, std::pair<uint32_t,
        ...
        std::map < uint32_t,
                std::pair < uint32_t, uint32_t > > ::iterator
        it = index.begin();
    ...
    Value value_parser(Value::kind k, string v) {
        switch (k) {
            case Value::I32:
                return Value((i32) stoi(v));

            case Value::I64:
                return Value((i64) stoi(v));

            case Value::DOUBLE:
                return Value((double) stof(v));
    ...
    void tbl_parser(string tbl_path, vector <vector<string>> &tbl) {
        string row_data;
        char *dup;
        char *token;
        vector <string> split_row_data;
        ...
        while (!infile.eof()) {
            ...
            getline(infile, row_data);
            dup = strdup(row_data.c_str());
            token = strtok(dup, "|");
            while (token != NULL) {
                split_row_data.push_back(string(token));
                token = strtok(NULL, "|");
            }
            tbl.push_back(split_row_data);
        }
        tbl.pop_back();
    }
}
