...
class ClientControlServiceImpl: public ClientControlService {
 private:
  ...
  typedef struct txn_info_t {
    ...
    std::vector<double> *interval_crt_latencies[2];
    std::vector<double> *interval_irt_latencies[2];


    std::vector<double> *last_interval_irt_latencies[2];
    std::vector<double> *last_interval_crt_latencies[2];

    std::vector<double> *interval_attempt_latencies[2];
    std::vector<int32_t> *num_try[2];

    std::vector<double> interval_irt_latency;
    std::vector<double> interval_crt_latency;
    ...
    txn_info_t() {
      ...
      interval_irt_latencies[0] = new std::vector<double>();
      interval_crt_latencies[0] = new std::vector<double>();

      interval_irt_latencies[1] = new std::vector<double>();
      interval_crt_latencies[1] = new std::vector<double>();

      last_interval_irt_latencies[0] = new std::vector<double>();
      last_interval_crt_latencies[0] = new std::vector<double>();

        last_interval_irt_latencies[1] = new std::vector<double>();
        last_interval_crt_latencies[1] = new std::vector<double>();
    ...
    void destroy() {
      delete interval_irt_latencies[0];
      delete interval_crt_latencies[0];
      delete interval_irt_latencies[1];
      delete interval_crt_latencies[1];

      delete last_interval_irt_latencies[0];
      delete last_interval_irt_latencies[0];
      delete last_interval_crt_latencies[1];
      delete last_interval_crt_latencies[1];
    ...
    void succ(bool switzh, latency_collection_status_t lcs, bool is_irt, double latency, double attempt_latency, int32_t tried) {
      total_txn++;
      total_try++;
      commit_txn++;
      int use = 1;
      ...
      switch (lcs) {
        case LCS_THIS_PERIOD:
            if (is_irt){
                interval_irt_latencies[use]->push_back(latency);
            }else{
                interval_crt_latencies[use]->push_back(latency);
            }
          break;
        case LCS_LAST_PERIOD:
            if (is_irt){
                last_interval_irt_latencies[use]->push_back(latency);
            }else{
                last_interval_crt_latencies[use]->push_back(latency);
            }
          break;
        case LCS_IGNORE:
        default:
      ...
      if (is_irt){
          interval_irt_latency.push_back(latency);
      }else{
          interval_crt_latency.push_back(latency);
      }
    }
 ...
 public:
  ...
  inline void txn_success_one(unsigned int id,
    ...
    if (last_time_ < start_time)
      ...
      lcs = LCS_LAST_PERIOD;
    txn_info_[id][txn_type].succ(txn_info_switch_, lcs, is_irt, latency, attempt_latency, tried);
    pthread_rwlock_unlock(&collect_lock_);
  }
