...
void TpccPiece::RegNewOrder() {
    ...
    BEGIN_PIE(TPCC_NEW_ORDER, TPCC_NEW_ORDER_1, DF_REAL) {
        ...
        Log_info("TPCC_NEW_ORDER, piece: %d w_id: %x", TPCC_NEW_ORDER_1, cmd.input[TPCC_VAR_W_ID].get_i32());
    ...
    BEGIN_PIE(TPCC_NEW_ORDER, TPCC_NEW_ORDER_2, DF_REAL) {
        ...
        // R customer
        Log_info("TPCC_NEW_ORDER, piece: %d c_id: %d d_id: %d w_id: %d",
                 TPCC_NEW_ORDER_2,
                 cmd.input[TPCC_VAR_C_ID].get_i32(),
                 cmd.input[TPCC_VAR_D_ID].get_i32(),
                 cmd.input[TPCC_VAR_W_ID].get_i32());
    ...
    BEGIN_PIE(TPCC_NEW_ORDER, TPCC_NEW_ORDER_3, DF_FAKE) {
        ...
        Log_info("TPCC_NEW_ORDER, piece: %d", TPCC_NEW_ORDER_3);
        i32 oi = 0;
    ...
    BEGIN_PIE(TPCC_NEW_ORDER, TPCC_NEW_ORDER_4, DF_FAKE) {
        verify(cmd.input.size() >= 3);
        Log_info("TPCC_NEW_ORDER, piece: %d", TPCC_NEW_ORDER_4);
    ...
    *res = SUCCESS;
    ...
    verify(cmd.input.size() >= 3);
    Log_info("TPCC_NEW_ORDER, piece: %d", TPCC_NEW_ORDER_RS(I));
    mdb::MultiBlob mb(2);
    int32_t w_id;
    ...
    *res = SUCCESS;
    ...
    Log_info("TPCC_NEW_ORDER, piece: %d", TPCC_NEW_ORDER_WS(I));
    mdb::Row *r = NULL;
    mdb::MultiBlob mb(2);
    mb[0] = cmd.input[TPCC_VAR_I_ID(I)].get_blob();
    mb[1] = cmd.input[TPCC_VAR_S_W_ID(I)].get_blob();
    ...
    *res = SUCCESS;
    ...
    BEGIN_LOOP_PIE(TPCC_NEW_ORDER, TPCC_NEW_ORDER_WOL(0), 1000, DF_FAKE) {
        ...
        Log_info("TPCC_NEW_ORDER, piece: %d", TPCC_NEW_ORDER_WOL(I));
