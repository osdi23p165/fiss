...
void TpccTxnGenerator::GetNewOrderTxnReq(TxnRequest *req,
    ...
    Value c_id((i32) RandomGenerator::nu_rand(1022, 1, tpcc_para_.n_c_id_ - 1));
...
void TpccTxnGenerator::get_tpcc_payment_txn_req(
    ...
    if (RandomGenerator::percentage_true(40)) {
        ...
        by_last_name = true;
        string cLast = "";
        TpccTxnGenerator::randomCLast(cLast);
        req->input_[TPCC_VAR_C_LAST] =
                Value(cLast);
    } else {
        req->input_[TPCC_VAR_C_ID] =
                Value((i32) RandomGenerator::nu_rand(1022, 1, tpcc_para_.n_c_id_ - 1));
    }
    auto config = Config::GetConfig();
