...
void TpccPiece::RegDelivery() {
  ...
  BEGIN_PIE(TPCC_DELIVERY, TPCC_DELIVERY_0, DF_REAL) {
    ...
    mbl[1] = cmd.input[TPCC_VAR_D_ID].get_blob();
    mbh[1] = cmd.input[TPCC_VAR_D_ID].get_blob();
    mbl[2] = cmd.input[TPCC_VAR_W_ID].get_blob();
    mbh[2] = cmd.input[TPCC_VAR_W_ID].get_blob();
    ...
    if (rs.has_next()) {
      r = rs.next();
//      TPL_KISS_ROW(r);
      dtxn->ReadColumn(r, TPCC_COL_NEW_ORDER_NO_O_ID, &o_id, TXN_DEFERRED);
      output[TPCC_VAR_O_ID] = o_id;
    } 
//      verify(0);
//      TPL_KISS_NONE;
      output[TPCC_VAR_O_ID] = Value(2101);
    // TODO FIXME
//    if (r) {
...
//    }
  ...
  BEGIN_PIE(TPCC_DELIVERY, TPCC_DELIVERY_1, DF_NO) {
    ...
    mb[0] = cmd.input[TPCC_VAR_O_ID].get_blob();
...
//    mdb::MultiBlob mbl(4), mbh(4);
    ...
    mbl[2] = cmd.input[TPCC_VAR_W_ID].get_blob();
    mbh[2] = cmd.input[TPCC_VAR_W_ID].get_blob();
    mbl[0] = cmd.input[TPCC_VAR_O_ID].get_blob();
    mbh[0] = cmd.input[TPCC_VAR_O_ID].get_blob();
