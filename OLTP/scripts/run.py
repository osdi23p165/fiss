...
sys.path += os.path.abspath("/htap/OLTP/dast/rrr/pylib"),
sys.path += os.path.abspath("/htap/OLTP/dast/deptran"),
# print(sys.path)
# import simplerpc
from simplerpc import Client
from simplerpc.marshal import Marshal

from rcc_rpc import ServerControlProxy
from rcc_rpc import ClientControlProxy
...
result_file_name_base = "/htap/OLTP/dast/results/data"
...
class TxnInfo(object):
    def __init__(self, txn_type, txn_name, interest):
        ...
        self.mid_time = 0.0
        self.mid_irt_latencies = []
        self.mid_crt_latencies = []
        self.mid_all_irt_latencies = []
        self.mid_all_crt_latencies = []
    ...
    def push_res(self, start_txn, total_txn, total_try, commit_txn,
            this_irt_latencies, this_crt_latencies, 
            last_irt_latencies, last_crt_latencies,
            irt_latencies, crt_latencies,
        ...
        if self.mid_status == 0:
            ...
            # logger.info("during recording period!!! {}".format(self.txn_type))

            self.mid_irt_latencies.extend(irt_latencies)
            self.mid_all_irt_latencies.extend(self.mid_irt_latencies)
            self.mid_crt_latencies.extend(crt_latencies)
            self.mid_all_crt_latencies.extend(self.mid_crt_latencies)
    ...
    def get_res(self, interval_time, total_time, set_max,
        ...
        if all_interval_commits > 0:
            interval_ret = [str(round(interval_commits * 100.0 / all_interval_commits, 2)) + "%", self.txn_name, self.start_txn - self.pre_start_txn, self.total_txn - self.pre_total_txn, self.total_try - self.pre_total_try, interval_commits, interval_tps]
        else:
            interval_ret = ["----", self.txn_name, self.start_txn - self.pre_start_txn, self.total_txn - self.pre_total_txn, self.total_try - self.pre_total_try, interval_commits, interval_tps]
    ...
    def get_data(self, config, num_clients):
        commit_txn = self.mid_commit_txn - self.mid_pre_commit_txn

        return commit_txn, self.mid_time, self.mid_irt_latencies, self.mid_crt_latencies
    ...
    def print_mid(self, config, num_clients):
        ...
        if self.mid_retry_exhausted > 0:
            logger.warn("max retry exhaused for {} txns".format(self.mid_retry_exhausted))
            m = [max(self.mid_crt_latencies)]
            self.mid_all_crt_latencies.extend(m * self.mid_retry_exhausted)

        self.mid_irt_latencies.sort()
        self.mid_crt_latencies.sort()
        self.mid_all_irt_latencies.sort()
        self.mid_all_crt_latencies.sort()
        ...
        for percent in g_latencies_percentage:
            ...
            percent = percent*100
            key = str(percent)
            if len(self.mid_irt_latencies)>0:
                index = int(math.ceil(percent/100*len(self.mid_irt_latencies)))-1
                irt_latencies[key] = self.mid_irt_latencies[index]
            else:
                irt_latencies[key] = NO_VALUE

            if len(self.mid_crt_latencies)>0:
                index = int(math.ceil(percent/100*len(self.mid_crt_latencies)))-1
                crt_latencies[key] = self.mid_crt_latencies[index]
            else:
                crt_latencies[key] = NO_VALUE

            if len(self.mid_all_irt_latencies)>0:
                index = int(math.ceil(percent/100*len(self.mid_all_irt_latencies)))-1
                all_irt_latencies[key] = self.mid_all_irt_latencies[index]
            else:
                all_irt_latencies[key] = NO_VALUE

            if len(self.mid_all_crt_latencies)>0:
                index = int(math.ceil(percent/100*len(self.mid_all_crt_latencies)))-1
                all_crt_latencies[key] = self.mid_all_crt_latencies[index]
            else:
                all_crt_latencies[key] = NO_VALUE
        ...
        self.data['irt_latency'] = {}
        self.data['irt_latency'].update(irt_latencies)
        if len(self.mid_irt_latencies)>0:
            self.data['irt_latency']['min'] = self.mid_irt_latencies[0]
            self.data['irt_latency']['max'] = self.mid_irt_latencies[len(self.mid_irt_latencies)-1]
        else:
            self.data['irt_latency']['min'] = NO_VALUE
            self.data['irt_latency']['max'] = NO_VALUE

        self.data['crt_latency'] = {}
        self.data['crt_latency'].update(crt_latencies)
        if len(self.mid_crt_latencies)>0:
            self.data['crt_latency']['min'] = self.mid_crt_latencies[0]
            self.data['crt_latency']['max'] = self.mid_crt_latencies[len(self.mid_crt_latencies)-1]
        else:
            self.data['crt_latency']['min'] = NO_VALUE
            self.data['crt_latency']['max'] = NO_VALUE

        self.data['all_irt_latency'] = {}
        self.data['all_irt_latency'].update(all_irt_latencies)
        if len(self.mid_all_irt_latencies)>0:
            self.data['all_irt_latency']['min'] = self.mid_all_irt_latencies[0]
            self.data['all_irt_latency']['max'] = self.mid_all_irt_latencies[len(self.mid_all_irt_latencies)-1]
        else:
            self.data['all_irt_latency']['min'] = NO_VALUE
            self.data['all_irt_latency']['max'] = NO_VALUE

        self.data['all_crt_latency'] = {}
        self.data['all_crt_latency'].update(all_crt_latencies)
        if len(self.mid_all_crt_latencies)>0:
            self.data['all_crt_latency']['min'] = self.mid_all_crt_latencies[0]
            self.data['all_crt_latency']['max'] = self.mid_all_crt_latencies[len(self.mid_all_crt_latencies)-1]
        else:
            self.data['all_crt_latency']['min'] = NO_VALUE
            self.data['all_crt_latency']['max'] = NO_VALUE
...
class ClientController(object):
    ...
    def client_run(self, do_sample, do_sample_lock):
        ...
        for site in sites:
            site.connect_rpc(3000)
            logger.debug("Connected to client site %s @ %s", site.name, site.process.host_address)
        ...
        try:
            ...
            logger.debug("Duration: {:.2f} seconds".format(time.time() - start))
    ...
    def start_client(self):
        ...
        logger.debug("client start send successfully.")
    ...
    def benchmark_record(self, do_sample, do_sample_lock):
        ...
        logger.debug("contact {} client rpc proxies".format(len(rpc_proxy)))
            ...
            for future in futures:
                ...
                for txn_type in res.txn_info.keys():
                    ...
                    self.txn_infos[txn_type].push_res(
                        ...
                        res.txn_info[txn_type].this_irt_latency,
                        res.txn_info[txn_type].this_crt_latency,
                        res.txn_info[txn_type].last_irt_latency,
                        res.txn_info[txn_type].last_crt_latency,
                        res.txn_info[txn_type].interval_irt_latency,
                        res.txn_info[txn_type].interval_crt_latency,
    ...
    def print_stage_result(self, do_sample, do_sample_lock):
        ...
        if (not self.recording_period):
            if (total_time >= 1 and total_time < 10):
                not_reported = True
            ...
            if (total_time >= 10):
                # this will generate more stable result
                logger.info("exit record")
                self.recording_period = False
                do_sample_lock.acquire()
                do_sample.value = 1
                do_sample_lock.release()
        ...
        if (total_time >= 10 and not_reported):
            not_reported = False
            commit_txn = 0
            time = 0    
            irt_latencies = []
            crt_latencies = []
            for k, v in self.txn_infos.items():
                ntxn, t, irt_lat, crt_lat = v.get_data(self.config, self.num_proxies)
                commit_txn += ntxn
                time = t
                irt_latencies.extend(irt_lat)
                crt_latencies.extend(crt_lat)
            irt_latencies.sort()
            crt_latencies.sort()

            

            tps = commit_txn / time * self.num_proxies
            logger.info("result:------------------------------------------------")
            logger.info("TPS: {}".format(tps))

            percent = 50.0
            index = int(math.ceil(percent/100*len(irt_latencies)))-1
            irt_med = irt_latencies[index]
            logger.info("IRT median latency: {}".format(irt_med))

            percent = 90.0
            index = int(math.ceil(percent/100*len(irt_latencies)))-1
            irt_90 = irt_latencies[index]
            logger.info("IRT 90% latency: {}".format(irt_90))

            percent = 95.0
            index = int(math.ceil(percent/100*len(irt_latencies)))-1
            irt_95 = irt_latencies[index]
            logger.info("IRT 95% latency: {}".format(irt_95))

            percent = 99.0
            index = int(math.ceil(percent/100*len(irt_latencies)))-1
            irt_99 = irt_latencies[index]
            logger.info("IRT 99% latency: {}".format(irt_99))
                
            percent = 50.0
            index = int(math.ceil(percent/100*len(crt_latencies)))-1
            crt_med = crt_latencies[index]
            logger.info("CRT median latency: {}".format(crt_med))

            percent = 90.0
            index = int(math.ceil(percent/100*len(crt_latencies)))-1
            crt_90 = crt_latencies[index]
            logger.info("CRT 90% latency: {}".format(crt_90))

            percent = 95.0
            index = int(math.ceil(percent/100*len(crt_latencies)))-1
            crt_95 = crt_latencies[index]
            logger.info("CRT 95% latency: {}".format(crt_95))

            percent = 99.0
            index = int(math.ceil(percent/100*len(crt_latencies)))-1
            crt_99 = crt_latencies[index]
            logger.info("CRT 99% latency: {}".format(crt_99))
            logger.info("-----------------------------------------------------")

            result_file = ""
            if trial_name == "n_clients":
                result_file = result_file_name_base + "-" + trial_name + "-" + system_name + ".csv"
            else:
                # no diff
                result_file = result_file_name_base + "-" + trial_name + "-" + system_name + ".csv"
            ...
            if trial_name == "crt_ratio":
                x_header.append("crt_ratio")
                x_value.append(self.config["bench"]["tpcc_payment_crt_rate"])

            elif trial_name == "n_clients":
                x_header.append("client_per_region")
                n_client_site = len(ProcessInfo.get_sites(self.process_infos, SiteInfo.SiteType.Client))
                concurrency = self.config["n_concurrent"]
                n_regions = len(ProcessInfo.get_sites(self.process_infos, SiteInfo.SiteType.Server)) / self.config["n_shard_per_region"] / 3 
                logger.info("client site {}, concurrency {}, n_region {}".format(n_client_site, concurrency, n_regions))
                x_value.append(n_client_site * concurrency / n_regions)

            elif trial_name == "tpca":
                x_header.append("zipf")
                x_value.append(self.config["bench"]["coefficient"])
            
            elif trial_name == "scalability":
                x_header.append("n_regions")

                n_server_site = len(ProcessInfo.get_sites(self.process_infos, SiteInfo.SiteType.Server))
                x_value.append(n_server_site / N_SHARD_PER_REGION / 3)

            elif trial_name == "robust":
                x_header.append("100 +- x")
                x_value.append(self.config['args'].experiment_name[:2])

            logger.info("trial {} on {} finishes".format(x_header[0], x_value[0]))

            # with open(result_file, 'a+') as f:
            #     writer = csv.writer(f)
            #     if add_header:
            #         x_header.extend(["tput", "irt_med", "irt_90", "irt_95", "irt_99", "crt_med", "crt_90", "crt_95", "crt_99"])
            #         writer.writerow(x_header)
            #     x_value.extend([tps, irt_med, irt_90, irt_95, irt_99, crt_med, crt_90, crt_95, crt_99])
            ...
            interval_header = ["RATIO", "NAME", "start", "finish", "attempts", "commits", "TPS"]
    ...
    def client_kill(self):
        ...
        ps.killall(hosts, "ROCOCO_SERVER", "-9")
...
class ServerController(object):
    ...
    def server_heart_beat(self, cond, s_init_finish, do_sample, do_sample_lock):
        ...
        try:
            ...
            for site in sites:
                site.connect_rpc(3000)
                logger.debug("Connected to site %s @ %s", site.name, site.process.host_address)

            for site in sites:

                logger.debug("call sync_server_ready on site {}".format(site.id))
                while (site.rpc_proxy.sync_server_ready() != 1):
                    time.sleep(1) # waiting for server to initialize
                logger.debug("site %s ready", site.name)
    ...
    def start(self):
        ...
        def run_one_server(process, process_name, host_process_counts):
            logger.debug("starting %s @ %s", process_name, process.host_address)
...
def create_parser():
    ...
    parser.add_argument("-e", "--exp", dest="trial_name",
                        help="the experiment to run, must be one of \"n_clients\", \"crt_ratio\", \"scalability\", \"tpca\"",
                        default = "", 
                        required= True)
    ...
    def connect_rpc(self, timeout):
        ...
        if self.site_type == SiteInfo.SiteType.Client:
            if self.process.client_rpc_proxy is not None:
                logger.debug("client control rpc already connected for site %s",
                             self.name)
                self.rpc_proxy = self.process.client_rpc_proxy
                return True
            logger.debug("start connect to client ctrl rpc for site %s @ %s:%s",
                     self.name,
                     self.process.host_address,
                     self.process.rpc_port)
            port = self.process.rpc_port
        else:
            logger.debug("start connect to server ctrl rpc for site %s @ %s:%s",
                     self.name,
                     self.process.host_address,
                     self.rpc_port)
            port = self.rpc_port
...
def build_config(options):
    ...
    for cf in config_files:
        for con in containers['containers']:
            ...
            target = con + ":/htap/OLTP/dast/config/" + base
            scp_command = ['scp', cf, target]
            print(scp_command)
            subprocess.call(scp_command)

    # if "mode" in config:
    #     print("mode {}".format(config["mode"]["cc"]))
    # else:
...
def main():
    ...
    try:
        ...
        global system_name
        system_name = config["mode"]["cc"]

        global trial_name 
        trial_name = config['args'].trial_name


        if system_name == None or trial_name == None:
            logger.exception("trial name or sys name not provided")

        process_infos = get_process_info(config)
        server_controller = ServerController(config, process_infos)
        logger.info("before server_controller.start");
        server_controller.start()
        logger.info("after server_controller.start");
